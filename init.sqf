...
// Initialise all shared scripts
[] call compileFinal preprocessFileLineNumbers "scripts\shared\initialise_shared_script_library.sqf";

// If machine is server then call server initialisation scripts
if (isServer) then {
	[] call compileFinal preprocessFileLineNumbers "scripts\server\initialise_server.sqf";
};

// If machine is headless client then call headless client initialisation scripts
if (!isDedicated && !hasInterface && isMultiplayer) then {
	execVM "scripts\server\offloading\headless_client_manager.sqf";
};

// Finally, if the machine is a player machine then wait until the player spawns and initialise the client scripts
